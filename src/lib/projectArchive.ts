...
export function buildArchive(projectId: string): ArchiveBundle | null {
  ...
  for (const page of pages) {
    ...
    const base = slug(page.title || tr("Sans titre", "Untitled"));
  ...
  const titres = new Map(useBlocksStore.getState().blocks.map((p) => [p.id, p.title || tr("Sans titre", "Untitled")]));
  ...
  for (const page of pages) {
    ...
    files.push({
      ...
      contents: `# ${page.title || tr("Sans titre", "Untitled")}\n\n${corps}\n`,
    });
  }
...
function sommaire(nomProjet: string, pages: Block[], fichiers: Map<string, string>): string {
  ...
  const parcourir = (parentId: string | null, profondeur: number) => {
    for (const page of pages.filter((p) => p.parentId === parentId).sort(parPosition)) {
      ...
      lignes.push(`${marge}- [${page.title || tr("Sans titre", "Untitled")}](${fichiers.get(page.id)})`);
      parcourir(page.id, profondeur + 1);
    }
  };
  parcourir(null, 0);

  if (lignes.length === 2) lignes.push(tr("_Ce projet ne contient aucune page._", "_This project has no pages._"));
  lignes.push(
    "",
    "---",
    "",
    tr(
      ...
      "_Projekt export. The Markdown files in this folder are for reading and sharing:"
    ),
    tr(
      "la mise en forme fine (polices, tailles, placement des images, rognage) n'y figure pas._",
      "fine formatting (fonts, sizes, image placement, cropping) isn't included._"
    ),
    tr(
      "_Pour restaurer le projet dans Projekt, importer `projekt.json`, pas ces fichiers._",
      "_To restore the project in Projekt, import `projekt.json`, not these files._"
    )
  );
  return lignes.join("\n");
}
...
export function parseArchive(brut: string): ArchivePayload {
  ...
  try {
    ...
    return refuser(tr("Ce fichier n'est pas un JSON valide.", "This file isn't valid JSON."));
  }

  const p = donnees as Partial<ArchivePayload>;
  if (!p || typeof p !== "object") return refuser(tr("Contenu illisible.", "Unreadable content."));
  if (p.format !== ARCHIVE_FORMAT) {
    return refuser(tr("Ce fichier n'est pas une sauvegarde Projekt.", "This file isn't a Projekt backup."));
  }
  if (typeof p.version !== "number" || p.version > ARCHIVE_VERSION) {
    return refuser(
      tr(
        ...
        `Backup format version ${String(p.version)}, newer than this copy of Projekt (${ARCHIVE_VERSION}).`
      )
    );
  }
  if (!Array.isArray(p.pages)) return refuser(tr("La sauvegarde ne contient aucune page.", "The backup contains no pages."));

  return {
    format: ARCHIVE_FORMAT,
    version: p.version,
    exportedAt: typeof p.exportedAt === "number" ? p.exportedAt : Date.now(),
    project: { name: p.project?.name?.trim() || tr("Projet importé", "Imported project") },
    pages: p.pages,
